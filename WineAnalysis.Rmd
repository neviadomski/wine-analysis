...
Now I want to plot histograms of all variables to get an idea about its distribution and decide for the things I will have a look deeper later. First 3 histograms are clear and informative. 
    
```{r echo=FALSE, message=FALSE, warning=FALSE, Building_Plots}
#Building plots
ggplot(aes(fixed.acidity), data = wines) + geom_histogram(binwidth = 0.2)
ggplot(aes(volatile.acidity), data = wines) + geom_histogram(binwidth = 0.01)
ggplot(aes(citric.acid), data = wines) + geom_histogram(binwidth = 0.05)
```

Though distribution of residual.sugar variable on the fourth plot doesn't look approximately normally distibuted. But we can modify it by taking log of our variable.
      
```{r echo=FALSE, message=FALSE, warning=FALSE, Building_Plots_2}
ggplot(aes(residual.sugar), data = wines) + geom_histogram(binwidth = 1)
ggplot(aes(residual.sugar), data = wines) + geom_histogram(binwidth = 0.1) + scale_x_log10()
#Adding log modified residual.sugar to dataset
wines$log_residual.sugar <- log(wines$residual.sugar)
```

Variables on the next 4 plots have many outlaiers, so I've cut them to get more clear and detailed picture.

```{r echo=FALSE, message=FALSE, warning=FALSE, Building_Plots_3}
ggplot(aes(chlorides), data = wines) + geom_histogram(binwidth = 0.001) +
    xlim(c(0,quantile(wines$chlorides, .98)))
ggplot(aes(free.sulfur.dioxide), data = wines) + geom_histogram(binwidth = 1) + 
    xlim(c(0,quantile(wines$free.sulfur.dioxide, .99)))
ggplot(aes(total.sulfur.dioxide), data = wines) + geom_histogram(binwidth = 5) +
    xlim(c(0,quantile(wines$total.sulfur.dioxide, .995)))
ggplot(aes(density), data = wines) + geom_histogram(binwidth = 0.0001) + 
    xlim(c(0.98,quantile(wines$density, .999)))
```

We don't need to modify last 4 plots. 

```{r echo=FALSE, message=FALSE, warning=FALSE, Building_Plots_4}
ggplot(aes(pH), data = wines) + geom_histogram(binwidth = 0.01)
ggplot(aes(sulphates), data = wines) + geom_histogram(binwidth = 0.01)
ggplot(aes(alcohol), data = wines) + geom_histogram(binwidth = 0.1)
ggplot(aes(quality), data = wines) + geom_bar()
```

I transformed residual sugar and cut outliers of chlorides, free.sulfur.dioxide, total.sulfur.dioxide and density. Now we can see patterns of our data more clearly. Main conclusion of this section is that distribution of most variable are bell-shaped (approximately normally distributed). 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Correlation_matrix, fig.width = 9, fig.height= 9}
#Building correlation matrix using corrplot library
cor_wines <- cor(subset(wines, select = -c(quality)))
legend_color <- colorRampPalette(c("navy", "deepskyblue", "darkblue"))
corrplot(cor_wines, method="number", col = legend_color(20), tl.col = "grey40")
```
...
ggpairs(wines_sample, columnLabels = c("Fix Acid","Vol Acid","Cit Acid","Res Sugar",
                                       "Chlorides","Free SO2","Total SO2","Density","pH",
                                       "Sulphates","Alcohol","Quality factor","Quality", "Log Sugar")) + 
    theme(axis.ticks = element_blank(), axis.text = element_blank())
```

After seeing correlation and plot matrices we can observe highest correlations in pairs residual.sugar - density, density - alcohol, free.sulfur.dioxide - total.sulfur.dioxide, fixed.acidity - pH. Let's make scatterplots of these pairs.

```{r echo=FALSE, Higher_correlation_variables_plots}
ggplot(aes(x = residual.sugar, y = density), data = subset(wines, residual.sugar < quantile(wines$residual.sugar, .999))) + 
    geom_point(alpha = 0.1) + geom_smooth()

ggplot(aes(x = density, y = alcohol), data = subset(wines, density < quantile(wines$density, .999))) +
    geom_point(alpha = 0.1) + geom_smooth()

ggplot(aes(x = free.sulfur.dioxide, y = total.sulfur.dioxide), data = 
           subset(wines, free.sulfur.dioxide < quantile(wines$free.sulfur.dioxide, .995))) +
    geom_point(alpha = 0.1) + geom_smooth()

ggplot(aes(x = fixed.acidity, y = pH), data = subset(wines, fixed.acidity < quantile(wines$fixed.acidity, .99))) +
...
ggplot(aes(x = density, y = quality), data = subset(wines, density < quantile(wines$density, .999))) +
    geom_jitter(height = 0.25, alpha = 0.05)
ggplot(aes(y = density, x = quality), data = subset(wines, residual.sugar < quantile(wines$residual.sugar, .999))) +
    geom_boxplot()
...
# 3factor plots
ggplot(aes(y = density, x = alcohol, color = residual.sugar), data = 
           subset(wines, density < quantile(wines$density, .999))) + geom_point()
...
# 3factor plots
ggplot(aes(y = density, x = total.sulfur.dioxide, color = fixed.acidity), data = 
           subset(wines, density < quantile(wines$density, .999))) + geom_point()
ggplot(aes(y = density, x = total.sulfur.dioxide, color = fixed.acidity.bin), data = 
           wines) + geom_smooth()
...
ggplot(aes(y = quality, x = total.sulfur.dioxide), data = wines) +
    facet_grid(density.bin ~ alcohol.bin) + geom_point(alpha = 0.1)
```
...
ggplot(aes(quality), data = wines) + geom_bar(fill = I('blue')) + 
    labs(x = "Quality (scale from 1 to 10)", y = "Frequency", title = "Quality distribution") 
...
ggplot(aes(y = alcohol, x = quality), data = wines) + geom_boxplot(color = I('blue'), outlier.colour=I("black")) + 
    labs(x = "Quality (scale from 1 to 10)", y = "Alcohol (% by volume)", title = "Alcohol distribution by Quality") + 
...
ggplot(aes(y = density, x = alcohol, color = residual.sugar), data = subset(wines, density < quantile(wines$density, .999))) + 
    geom_point(alpha = 0.5) + scale_colour_gradient(low = "#33ccff", high = "#0000cc", name = "Residual sugar (g / dm^3)") + 
    labs(x = "Alcohol (% by volume)", y = "Density (g / cm^3)", title = "Alcohol and Sugar impact on Density of wine")
```
